...
### 工作记录
- **周报管理** — 按周记录工作内容，自动生成周报
- **任务同步** — 已完成的任务自动同步到工作记录
- **条目管理** — 支持添加、编辑、删除、取消工作条目
- **详情查看** — 点击工作记录可查看详情
...
### 体验细节
- **命令面板** — 全局快捷键 `Ctrl/Cmd + K` 唤起命令面板，预加载数据，快速搜索凭证/任务，直接弹出详情无需跳转
...
|------|------|------|
| id | text | 主键 (nanoid) |
| week_start | timestamp | 周起始日期 |
| week_end | timestamp | 周结束日期 |
| project_progress | text | 项目进度 |

### 工作记录条目 (work_log_items)

| 字段 | 类型 | 说明 |
|------|------|------|
| id | text | 主键 (nanoid) |
| work_log_id | text | 关联工作记录 |
| content | text | 条目内容 |
| is_cancelled | boolean | 是否已取消 |
| sort_order | int | 排序序号 |
| source_task_id | text | 关联任务ID |
